...
class Page152TunerApp:
    ...
    def _draw_plot(self) -> None:
        ...
        self.ax.plot(self.disp_dates, lp_scaled, "b--", linewidth=1.4, alpha=0.8, zorder=2)
        self.ax.text(
            0.01,
        ...
        for out_idx, bp_name, bp_label in zip(range(1, 6), BP_NAMES, BP_LABELS):
            ...
            if np.any(np.isnan(sig)):
                ...
                self.ax.plot(self.disp_dates[valid], sig_scaled[valid], "k.", markersize=2, zorder=2)
            else:
                self.ax.plot(self.disp_dates, sig_scaled, "k-", linewidth=0.7, zorder=2)
