...
kernels_real = create_mixed_kernels(FILTER_SPECS, FS, method='modulate',
                                     bp_analytic=False)
outputs_real = apply_and_collect(kernels_real, close_prices, FS)

fig1 = plot_decomposition(
    outputs_real, disp_dates, s_idx, e_idx,
    title="Page 152: Six-Filter Decomposition — Real-Valued Filters",
    show_envelope=False,
    filename='page152_real.png'
)
...
kernels_mod = create_mixed_kernels(FILTER_SPECS, FS, method='modulate',
                                    bp_analytic=True)
outputs_mod = apply_and_collect(kernels_mod, close_prices, FS)

fig2 = plot_decomposition(
    outputs_mod, disp_dates, s_idx, e_idx,
    title="Page 152: Six-Filter Decomposition — Complex Modulated + Envelopes",
    show_envelope=True,
    filename='page152_complex_modulate.png'
)
...
kernels_sub = create_mixed_kernels(FILTER_SPECS, FS, method='subtract',
                                    bp_analytic=True)
outputs_sub = apply_and_collect(kernels_sub, close_prices, FS)

fig3 = plot_decomposition(
    outputs_sub, disp_dates, s_idx, e_idx,
    title="Page 152: Six-Filter Decomposition — Complex Subtract + Envelopes",
    show_envelope=True,
    filename='page152_complex_subtract.png'
)
